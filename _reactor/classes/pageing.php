...
OUT;
	...
	public function GetAdminHTML(){
        ...
        $get_url=str_replace("index.php", "", $get_url);
        echo $get_url."<br>";*/
	...
	/*unset($this->url[0]);
        ...
        //if(!empty($_SERVER['QUERY_STRING']))
            ...
            unset($QUERY_STRING_ARR[0]);
            if(count($QUERY_STRING_ARR)>0)
            $q_string="?".implode('&',$QUERY_STRING_ARR);
	...
	$str .=<<<OUT
			...
			<span class="page_no">Страница {$this->page} из {$this->total}</span>
		  	<ul class="pag_list">
OUT;
	if($prev>0){
	$str .=<<<OUT
			<li class="prev"><a href="{$this->out_url}{$prev}/{$q_string}"><i class="i"></i></a></li>
OUT;
	}
	...
	for ($i=1; $i < $this->page ; $i++)
		{
			if ($i>((int)$this->page-(int)$this->half_block)){
				if($i==1){
				$str .=<<<OUT
				<li><a href="{$this->out_url}{$this->list[$i]}/{$q_string}">{$this->list[$i]}</a></li>
OUT;
				}
				else {
				$str .=<<<OUT
	...
	for ($i=$this->page+1; $i <= $this->total_pages ; $i++)
		{
			...
			break;
			$str .=<<<OUT
			<li{$classlast}><a href="{$this->out_url}{$this->list[$i]}/{$q_string}">{$this->list[$i]}</a></li>
OUT;
			$cnt++;
		}
	...
	if($this->list['current']<$this->total_pages){
	$str .=<<<OUT
			<li class="next"><a href="{$this->out_url}{$next}/{$q_string}"><i class="i"></i></a></li>
OUT;
	}
