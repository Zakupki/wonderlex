<?php
    ...
    class smtp {
         ...
         */
        function Data($msg_data, $title=null,$sendername=null) {
            $this->error = null; # so no confusion is caused
            if(!$sendername)
            $sendername=MAIN_NAME;
            $header="Date: ".date("D, j M Y G:i:s")." +0300\n"; 
            $header.="From: =?utf-8?Q?".str_replace("+","_",str_replace("%","=",urlencode($sendername)))."?= <".$this->sender.">\n"; 
            $header.="X-Mailer: The Bat! (v3.99.3) Professional\n"; 
            $header.="Return-Path: <".$this->sender.">\n";
            $header.="Reply-To: =?utf-8?Q?".str_replace("+","_",str_replace("%","=",urlencode($sendername)))."?= <".$this->sender.">\n";
            $header.="X-Priority: 3 (Normal)\n";
            $header.="Message-ID: <172562218.".date("YmjHis")."@reactor-pro.ru>\n";
            $header.="To: ".implode(',',$this->recipientArr)."\n";
        ...
        function Recipient($to) {
            if(!in_array($to,$this->recipientArr))
            $this->recipientArr[]="<".$to.">";
