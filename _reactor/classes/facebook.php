...
Class Facebook {
	...
	public function __construct(){
        ...
        $this->canvas_page2="http://wonderlex.com/social/fbconnect/";
		$this->my_url="http://wonderlex.com/facebook/connect/";
        $this->my_url2="http://wonderlex.com/social/fbconnect/";
	}
	...
	public function connect2($sescode){
		   ...
		   if($_SESSION['state'] && ($_SESSION['state'] === $_REQUEST['state'])) {
		     ...
		     $token_url = "https://graph.facebook.com/oauth/access_token?"
		       . "client_id=" . $this->app_id . "&redirect_uri=" . urlencode($this->my_url2)
    ...
    function addAccount2($data, $params){
       ...
       /* $avatar=tools::GetFacebookImageFromUrl('http://graph.facebook.com/'.$data->id.'/picture?type=large');
        ...
        if(!$siteid)
            $siteid=1;
        $db->exec('INSERT INTO z_social_account (socialid, accountid,token,tokenexpires,siteid,name,email,file_name,userid)
		VALUES (255,'.$data->id.',"'.$params['access_token'].'",'.$params['expires'].','.$siteid.',"'.$data->name.'", "'.$data->username.'@wonderlex.com","'.$newfile.'", '.tools::int($newuserid).')');
