...
VK.init = function(options) {
  ...
  if (options.vk) {
    VK._domain.main = 'https://oauth.vk.com/';
  }
  ...
  VK.Api = {
    ...
    checkMethod: function(method, params, cb, queryTry) {
      ...
      if (m == 'wall.post' || m == 'activity.set') {
        ...
        var query =  (location.protocol == 'https:' ? 'https:' : '') + '//vk.com/al_apps.php?act=wall_post_box&widget=1&method='+m+'&aid=' + parseInt(VK._apiId, 10) + '&text=' + encodeURIComponent(text);
        if (m == 'wall.post') {
  ...
  VK.UI = {
    ...
    button: function(el, handler) {
      ...
      html = (
        ...
        '<td><div style="background: url(//vk.com/images/btns.png) 0px -42px no-repeat; width: 21px; height: 21px"></div></td>' +
...
if (!VK.Widgets) {
  ...
  VK.Widgets.loading = function(obj, enabled) {
    obj.style.background = enabled ? 'url("//vk.com/images/upload.gif") center center no-repeat transparent' : 'none';
  }
  ...
  VK.Widgets.Comments = function(objId, options, page) {
    ...
    return VK.Widgets._constructor('widget_comments.php', objId, options, params, {
      showBox: function (url, props) {
        var box = VK.Util.Box((options.base_domain || '//vk.com/') + url, [props.width, props.height], {
          proxy: function () {
...
console.log(params);
  ...
  VK.Widgets.Ads = function(objId, options, paramsExtra) {
    ...
    params.domain_id = paramsExtra.domain_id;
    params.hash      = paramsExtra.hash;
    options.noDefaultParams = true;
    options.width           = 118;
    defaults.startHeight = 100;
    defaults.minWidth    = 118;
    funcs.adsCheckVisible = adsCheckVisible;
    funcs.adsAddEvents    = adsAddEvents;

    return VK.Widgets._constructor('ads_rotate.php', objId, options, params, funcs, defaults, onDone);

    function adsCheckVisible(oneAdMaxSize) {
      var top             = (VK.Util.getXY(iframe) || {})[1];
      var scrollTop       = scrollGetY();
      var windowHeight    = Math.max(parseInt(window.innerHeight) || 0, parseInt(document.documentElement.clientHeight) || 0);
      var isVisibleIframe = ((top + oneAdMaxSize > scrollTop) && (top < scrollTop + windowHeight));
      rpc.callMethod('onAdsCheckVisibleResult', isVisibleIframe);
    }
    function adsAddEvents(adsScriptVersion) {
      ...
      var isAlreadyAttached = adsAttachScript(adsScriptVersion);
      if (isAlreadyAttached && window.vk__adsLight && vk__adsLight.onInit) {
        vk__adsLight.onInit();
      }
    }
    function adsAttachScript(adsScriptVersion) {
      ...
      var pattern = new RegExp('^https?://(?:(?:[a-z0-9-]+)\.)*vk.com/js/al/aes_light\\.js\\?\\d+$', 'i');
      var found = false;
      for (var i = 0, elem; elem = scripts[i]; i++) {
        if (elem.src.match(pattern)) {
          found = true;
          break;
        }
      }
      if (!found) {
        VK.Api.attachScript('http://vk.com/js/al/aes_light.js?' + adsScriptVersion);
      }
      return found;
    }
  ...
  VK.Widgets._constructor = function(widgetUrl, objId, options, params, funcs, defaults, onDone, widgetId, iter) {
    ...
    base_domain = options.base_domain || '//vk.com';
...
VK.Util = {
  ...
  scrollGetY: function() {
    return window.pageYOffset || window.scrollNode && scrollNode.scrollTop || document.documentElement.scrollTop;
  },
  Box: function (src, sizes, fnc, options) {
    ...
    iframe = rpc.append(document.body, {
      ...
      scrolling: 'no',
      allowTransparency: true,
      style: {position: 'absolute', left: '50%', zIndex: 1002, background: '//vk.com/images/upload.gif center center no-repeat transparent', padding: '0', border: '0', width: sizes[0] + 'px', height: sizes[1] + 'px', overflow: 'hidden'}
    }),
